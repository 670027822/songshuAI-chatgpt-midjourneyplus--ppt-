...
export class MidjourneyController {
  ...
  @Get('tishici')
  @ApiOperation({ summary: '获取提示词' })
  @ApiBearerAuth()
  async tishici(@Query() params: { fenlei: String,skip:number,take:number }) {
    return await this.midjourneyService.tishicihuoqu(params);
  }

  
  @Get('huihuaeg')
  @ApiOperation({ summary: '获取绘画示例' })
  @ApiBearerAuth()
  async huihuaeg() {
    return await this.midjourneyService.huihuaeghuoqu();
  }
}
