...
export class AppService {
  ...
  // 测试1
  async collect(body: CollectAppDto, req: Request): Promise<string> {
    try {
      const { appId } = body;
      const { id: userId } = req.user;
      const hasCollected = await this.checkIfAppIsCollected(appId, userId);

      if (hasCollected) {
        await this.removeCollection(appId, userId);
        return '取消收藏成功!';
      } else {
        await this.addCollection(appId, userId);
        return '已将应用加入到我的个人工作台！';
      }
    } catch (error) {
      // 对于捕捉到的任何异常，可以使用适当的异常处理逻辑
      throw new HttpException('处理收藏操作时发生错误', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  private async checkIfAppIsCollected(appId, userId): Promise<boolean> {
    const historyApp = await this.userAppsEntity.findOne({ where: { appId, userId } });
    return !!historyApp;
  }

  private async removeCollection(appId, userId): Promise<void> {
    const deleteResult = await this.userAppsEntity.delete({ appId, userId });
    if (deleteResult.affected === 0) {
      throw new HttpException('取消收藏失败！', HttpStatus.BAD_REQUEST);
    }
  }

  private async addCollection(appId, userId): Promise<void> {
    const app = await this.appEntity.findOne({ where: { id: appId } });
    if (!app) {
