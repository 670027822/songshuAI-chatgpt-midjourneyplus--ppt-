...
export class ChatLogService {
  ...
  /* 语音 */
  async yuyinChatLog(req: Request, id: number) {
    console.log('id', id);
    console.log('id99999');
    const { id: userId } = req.user;
    console.log('查询对话');
    ...
    try {
      ...
      console.error('无法加载request模块。错误信息：', error.message);
      // 处理错误或退出程序
      process.exit(1); // 非零状态码表示异常退出
    }

    console.log('查询模型');
    const moxing = await this.modelsEntity.findOne({
      where: [{ model: 'tts-1-hd' }, { model: 'tts-1' }],
    });
    console.log('moxing', moxing);

    const key = moxing.key;
    const url = moxing.proxyUrl || 'https://api.openai.com';
    const voice = moxing.voice;
    const model = moxing.model;
    const fullurl = `${url}/v1/audio/speech`;
    ...
    var dataString = JSON.stringify({ model: model, input: r, voice: voice });
    var options = {
      url: fullurl,
      method: 'POST',
      headers: headers,
