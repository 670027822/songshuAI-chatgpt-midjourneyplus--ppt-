...
export class ChatLogController {
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  async yuyin(@Req() req: Request, @Res() res: Response, @Query('id') id: number) {
    // 生成一个唯一的缓存键，例如结合用户ID和查询ID
    const cacheKey = `${req.user.id}-${id}`;

    // 检查缓存中是否有对应的响应
    if (this.cache.has(cacheKey)) {
      // 缓存中有数据，则直接返回缓存的响应
      console.log('缓存中有数据，则直接返回缓存的响应');
      res.send(this.cache.get(cacheKey));
      return;
    }

    // 缓存中没有数据，进行API请求
    console.log('缓存中没有数据，进行API请求');
    const response = await this.chatLogService.yuyinChatLog(req, id);
    // 将响应存储到缓存中
    console.log('将响应存储到缓存中');
