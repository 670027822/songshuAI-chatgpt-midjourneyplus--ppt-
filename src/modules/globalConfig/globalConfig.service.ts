...
export class GlobalConfigService implements OnModuleInit {
  ...
  /* 查询流动公告信息 */
  async queryliudonggg() {
    return await this.getConfigs(['liudonggg']);
  }
  /* 查询流动公告信息 */
  async queryzhuti() {
    return await this.getConfigs(['morenzhuti']);
  }
  ...
  async auth() {
    const api = 'https://gpt.mylaws.top/auth.php';
    const a = process.env.SONG_AI_HOST;
    ...
    if (!success) {
      Logger.error('授权信息错误:', message);
      Logger.error('授权验证不通过，请联系官方客服v：670027822购买正版授权');
      process.exit(1);
    } else {
      Logger.debug('ip验证通过');
    }
  ...
  async checsongshu() {
    const a = process.env.SONG_AI_HOST;
    const b = process.env.SONG_AI_KEY;
    const crypto = require('crypto');
    const appendedString = a + 'H7D3NJ';
    // md5 16 位大写
    const hash = crypto.createHash('md5').update(appendedString, 'utf8').digest('hex').toUpperCase();
    const md5_16bit = hash.substring(8, 24);
    if (md5_16bit === b) {
      console.log('api验证通过');
    } else {
      console.log('秘钥验证不通过，请联系官方客服v：670027822购买正版授权');
      process.exit(1);
    }
  }
}
