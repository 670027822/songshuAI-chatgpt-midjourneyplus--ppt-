...
export class UploadService implements OnModuleInit {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        this.tencentCos.putObject(
          ...
          async (err, data) => {
            ...
            Logger.debug('locationUrl232342', locationUrl);
  ...
  async uploadFileByTencentCosFromUrl({ filename, url, dir }) {
    ...
    try {
      ...
      console.log('图片代理数据', data)
      Logger.debug('proxyMj', proxyMj);
      /* 开启代理 */
      if (Number(proxyMj) === 111) {
        const mjProxyUrl = (await this.globalConfigService.getConfigs(['mjProxyUrl'])) || 'http://172.247.48.137:8000';
        console.log('mjp130', mjProxyUrl)
        ...
        try {
          console.log('不走代理直接上传存储桶开始');

          try {
            console.log('buffer22')
            console.log('url', url)
            const buffer = await this.getBufferFromUrl(url);
            console.log('buffer4464')
          ...
          console.log('不走代理直接上传存储桶失败', error)
        }
