...
export class ChatgptController {
  ...
  async chatmind(@Body() body: ChatProcessDto, @Req() req: Request, @Res() res: Response) {
    ...
    body.systemMessage =
      mindCustomPrompt ||
      `你只能使用markdown格式回答我得问题、我的需求是得到一份markdown格式的大纲、尽量做的精细、层级多一点、不管我问你什么、都需要您回复我一个大纲出来。除了大纲之外，不要输出任何非markdown格式之外内容。记住，不要输出任何非markdown格式之外内容。也不要给大纲的前后添加任何多余的符号和提示说明。`;
  ...
  @ApiBearerAuth()
  async draw(@Body() body: ChatDrawDto, @Req() req: Request) {
    req.setTimeout(90000); // 设置超时时间为90秒
    return await this.chatgptService.draw(body, req);
  }
