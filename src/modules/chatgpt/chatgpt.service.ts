...
import { getTokenCount, sendMessageFromOpenAi, huoquurl } from './openai';
...
export class ChatgptService implements OnModuleInit {
  ...
  async chatProcess(body: any, req: Request, res?: Response) {
    ...
    // 接收并导出数据
    const { options = {}, appId, cusromPrompt, systemMessage = '', imgsup = '[]' } = body;
    /* 不同场景会变更其信息 */
    let setSystemMessage = systemMessage;
    const { parentMessageId } = options;
    const { prompt } = body;
    const { groupId, usingNetwork, usingduomo } = options;
    ...
    const { keyType, model, modelName, modeltopN: temperature, systemMessage: customSystemMessage, rounds } = groupConfig.modelInfo;
    /* 获取到本次需要调用的key */
    let currentRequestModelKey = null;

    if (!cusromPrompt) {
      console.log('model', model);
      currentRequestModelKey = await this.modelsService.getCurrentModelKeyInfo(model, modelName);
    } else {
    ...
    const { deduct, deductType, key: modelKey, secret, id: keyId, accessToken } = currentRequestModelKey;
    ...
    try {
      if (res) {
        ...
        if (Number(keyType) === 1) {
          ...
          let firstChunk = true;
          let imgsup3 = JSON.parse(imgsup);

          // 获取图片的URL，得到的url为json数组
          if (model === 'gpt-4-vision-preview' && imgsup3.length > 0) {
            imgsup3 = await huoquurl(imgsup3);
          }
  ...
  async draw(body: ChatDrawDto, req: Request) {
    ...
    await this.userService.checkUserStatus(req.user);
    // TODO 目前仅支持一张才这样计算
    const money = body?.size === '1024x1024' ? 2 : 4;
    ...
    try {
      ...
      for (const item of images) {
        ...
        // Check if the item has a base64 encoded string
        if (item.b64_json) {
