...
export class ModelsService {
  ...
  // 测试函数
  async getCurrentModelKeyInfo(model, modelName) {
    if (!this.keyPoolMap[model]) {
      throw new HttpException('当前调用模型已经被移除、请重新选择模型！', HttpStatus.BAD_REQUEST);
    }

    // 查找具有特定 modelName 的 ModelsEntity 对象
    const modelsEntities = this.keyPoolMap[model].filter((entity) => entity.modelName === modelName);
    if (modelsEntities.length === 0) {
      throw new HttpException(`模型 ${modelName} 在 ${model} 中找不到`, HttpStatus.BAD_REQUEST);
    }

    // 假设我们只关心找到的第一个匹配项
    const modelEntity = modelsEntities[0];

    // 逻辑修改为直接返回找到的 ModelsEntity 对象
    console.log('ModelsEntity', ModelsEntity);
    return modelEntity; // 或者 return modelEntity.key; 如果只需要 key 字段
  }
  ...
  async modelsList() {
    ...
    Object.keys(cloneModelMaps).forEach((key) => {
      cloneModelMaps[key] = Array.from(
        cloneModelMaps[key]
          .filter((t) => t.model !== 'tts-1-hd' && t.model !== 'tts-1') // 过滤掉不需要的 model 值
          .map((t) => {
            const { modelName, model, deduct, deductType, maxRounds } = t;
            return { modelName, model, deduct, deductType, maxRounds };
          })
