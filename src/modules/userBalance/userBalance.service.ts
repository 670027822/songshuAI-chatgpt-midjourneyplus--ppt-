...
export class UserBalanceService {
  ...
  async refundMjBalance(userId, amount) {
    ...
    // 找到对应的用户余额记录
    const userBalance = await this.userBalanceEntity.findOne({ where: { id: userId } });
    console.log('userBalance35342', userBalance)

    // 确认用户存在并且drawMjCount大于等于要减少的数值
    if (userBalance && userBalance.drawMjCount >= amount) {
      // 减少用户的drawMjCount
      userBalance.drawMjCount += amount;

      // 保存更新后的用户余额记录
      await this.userBalanceEntity.save(userBalance);
    } else {
      // 用户不存在或者drawMjCount不足，抛出错误或者进行其他处理
